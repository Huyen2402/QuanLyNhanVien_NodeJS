...
const ChucVuService = require("../services/chucvu.service.js");
...
exports.findAll = async (req, res, next) => {
  ...
  try {
    ...
    if (name) {
      documents = await chucVuService.findByName(name);
    } else {
...
exports.delete = async (req, res, next) => {
    try {
        const chucVuService = new ChucVuService(MongoDB.client);
        const document = await chucVuService.delete_id(req.params.id);
        if(!document){
            return next(new ApiError(404, "Phong not found"));
        }
